...
class XMLProcessor:
    ...
    def find_root_description(self):
        try:
            finded_root = self.root.findall('.//cbc:Description', namespaces={
                'cbc': UBL_NAMESPACE
            })

            return finded_root[0].text if finded_root else None
        except IndexError:
            return "XML sin etiquetas <cbc:Description>"

    def find_qr_code(self):
        try:
            qrcode_element = self.root.find('.//sts:QRCode', namespaces={
                'sts': DIAN_NAMESPACE
            })
            return qrcode_element.text if qrcode_element is not None else None
        except IndexError:
    ...
    def find_invoice_items(self):
        try:
            ...
            if invoice_items:
                items = [self.extract_invoice_line_info(item) for item in invoice_items]
                return items
            else:
...
class XMLFileCreator:
    def __init__(self, text, original_file_name: str):
        self.text = text
        self.original_file_name = original_file_name

    def create_file(self):
        prefix = self.original_file_name[:5]

        path_copies_file = os.path.join(
            'source', 'copies', f"copia-{prefix}.xml")
    ...
    def create_csv_file(self):
        try:
            ...
            with open(path_csv_file, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                data_list = [item if isinstance(item, dict) else dict(item) for item in data_list]
